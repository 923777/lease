...
public class ViewAppointmentController {
    ...
    public Result<IPage<AppointmentVo>> page(@RequestParam long current, @RequestParam long size, AppointmentQueryVo queryVo) {
        IPage<AppointmentVo> list = viewAppointmentService.pageAppointmentByQuery(new Page<>(current,size),queryVo);
    ...
    public Result updateStatusById(@RequestParam Long id, @RequestParam AppointmentStatus status) {
        viewAppointmentService.update(new LambdaUpdateWrapper<ViewAppointment>().eq(ViewAppointment::getId,id).set(ViewAppointment::getAppointmentStatus,status));
        return Result.ok();
    }
